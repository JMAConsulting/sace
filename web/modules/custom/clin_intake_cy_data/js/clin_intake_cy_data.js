jQuery(document).ready(function ($, settings) {
  ...
  $(
    `${drupalSettings.clin_intake_cy_data.start_date}-date, ${drupalSettings.clin_intake_cy_data.start_date}-time, ${drupalSettings.clin_intake_cy_data.end_date}-date, ${drupalSettings.clin_intake_cy_data.end_date}-time`
  ).on("input", function () {
    var end = $(`${drupalSettings.clin_intake_cy_data.end_date}-date`).val();
    var endTime = $(
      `${drupalSettings.clin_intake_cy_data.end_date}-time`
    ).val();
    var start = $(
      `${drupalSettings.clin_intake_cy_data.start_date}-date`
    ).val();
    var startTime = $(
      `${drupalSettings.clin_intake_cy_data.start_date}-time`
    ).val();
    if (end && start) {
