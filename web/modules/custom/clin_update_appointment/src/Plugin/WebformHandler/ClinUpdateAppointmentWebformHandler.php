...
namespace Drupal\clin_update_appointment\Plugin\WebformHandler;
 ...
 *
 * @WebformHandler(
 *   id = "clin_update_appointment_handler",
 *   label = @Translation("CLIN Update Appointment Handler"),
 *   category = @Translation("CRM"),
 *   description = @Translation("Webform Handler for fixing the CLIN Update Appointment Form"),
 *   cardinality = \Drupal\webform\Plugin\WebformHandlerInterface::CARDINALITY_SINGLE,
 *   results = \Drupal\webform\Plugin\WebformHandlerInterface::RESULTS_PROCESSED,
 *   submission = \Drupal\webform\Plugin\WebformHandlerInterface::SUBMISSION_OPTIONAL,
 * )
 */
class ClinUpdateAppointmentWebformHandler extends WebformHandlerBase {
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    ...
    if ($webform_submission_data) {
      // Do not create a new activty if appointment status is attended
      if($webform_submission_data['civicrm_1_activity_1_activity_status_id'] != 10) {
        if($webform_submission_data['civicrm_1_activity_1_activity_status_id'] != 14 && $webform_submission_data['civicrm_1_activity_1_activity_status_id'] != 15) {
          $results = \Civi\Api4\Activity::create(FALSE)
            ->addValue('source_record_id', $webform_submission_data['civicrm_1_contact_1_contact_existing'])
