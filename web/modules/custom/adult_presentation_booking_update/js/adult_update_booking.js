jQuery(document).ready(function ($, settings) {
  ...
  $(`${drupalSettings.adult_booking_update.start_date}-date, ${drupalSettings.adult_booking_update.start_date}-time, ${drupalSettings.adult_booking_update.end_date}-date, ${drupalSettings.adult_booking_update.end_date}-time`).on('input', function () {
    var end = $(`${drupalSettings.adult_booking_update.end_date}-date`).val();
    var endTime = $(`${drupalSettings.adult_booking_update.end_date}-time`).val();
    var start = $(`${drupalSettings.adult_booking_update.start_date}-date`).val();
    var startTime = $(`${drupalSettings.adult_booking_update.start_date}-time`).val();
    if (end && start) {
