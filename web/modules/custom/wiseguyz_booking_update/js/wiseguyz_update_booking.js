jQuery(document).ready(function ($, settings) {
  ...
  $(`${drupalSettings.wiseguyz_booking_update.start_date}-date, ${drupalSettings.wiseguyz_booking_update.start_date}-time, ${drupalSettings.wiseguyz_booking_update.end_date}-date, ${drupalSettings.wiseguyz_booking_update.end_date}-time`).on('input', function () {
    var end = $(`${drupalSettings.wiseguyz_booking_update.end_date}-date`).val();
    var endTime = $(`${drupalSettings.wiseguyz_booking_update.end_date}-time`).val();
    var start = $(`${drupalSettings.wiseguyz_booking_update.start_date}-date`).val();
    var startTime = $(`${drupalSettings.wiseguyz_booking_update.start_date}-time`).val();
    if (end && start) {
