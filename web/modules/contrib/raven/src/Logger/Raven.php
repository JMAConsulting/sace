...
class Raven implements LoggerInterface, RavenInterface {
  ...
  public function __construct(
    ...
    protected ?RequestFetcherInterface $requestFetcher = NULL,
  ) {
  ...
  public function log($level, string|\Stringable $message, array $context = []): void {
    ...
    if (!empty($log_levels[$level + 1]) && !in_array($context['channel'], $ignored_channels) && !in_array($unformatted_message, $ignored_messages)) {
      $levels = [
