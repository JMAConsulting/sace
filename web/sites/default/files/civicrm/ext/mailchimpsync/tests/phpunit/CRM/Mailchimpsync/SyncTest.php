...
class CRM_Mailchimpsync_SyncTest extends \PHPUnit\Framework\TestCase implements HeadlessInterface, HookInterface, TransactionalInterface {
   ...
   * Test tag sync.
   */
  public function testTagUpdates() {
    ...
    $bao->mailchimp_list_id = 'list_1';
    $bao->mailchimp_email = 'contact1@example.com';
    $bao->civicrm_groups = $audience->getSubscriptionGroup() . ";Added;2020-01-01";
    $bao->mailchimp_updated = '2020-01-01';
    $bao->mailchimp_status = 'subscribed';
    $bao->subscribeInCiviCRM($audience);
    $bao->save();
    $cache_id = $bao->id;
    ...
    // Now it's all set up, run reconciliation then test expected outcomes.
    //
    $audience->checkForDataUpdates(NULL);
    $bao = new CRM_Mailchimpsync_BAO_MailchimpsyncCache();
    $bao->id = $cache_id;
    $bao->find(TRUE);
    ...
    // ---------------------------------------------------------------
    // 3rd test: if no change to tags, should be no updates.
    $audience->checkForDataUpdates(NULL);
    $bao = new CRM_Mailchimpsync_BAO_MailchimpsyncCache();
    $bao->id = $cache_id;
    $bao->find(TRUE);
  ...
  public function testBatchWebhookHandlesComplianceFailures() {
    ...
    // Delete all tags, we don't want to test those.
    $tags = civicrm_api3('tag', 'get', ['used_for' => 'civicrm_contact'])['values'];
    foreach ($tags as $tag) {
      civicrm_api3('tag', 'delete', ['id' => $tag['id']]);
    }
   ...
   */
  public function unused_testTagUpdatesCanBeMadeViaHooks() {

    Civi::dispatcher()->addListener('hook_mailchimpsync_data_updates_check',
      [$this, 'hookForTagUpdatesCanBeMadeViaHooks']);

    // @todo: create a contact on a list, create the updates, inspect the updates. 3 times.
    $audience = $this->createConfigFixture1AndGetAudience(TRUE);
    // Create one test contact.
    $contact_1 = (int) civicrm_api3('Contact', 'create', [
      'contact_type' => 'Individual',
    ...
    //
    // do it again, run reconciliation then test expected outcomes.
    //
    $audience->checkForDataUpdates(NULL);
    $bao = new CRM_Mailchimpsync_BAO_MailchimpsyncCache();
    $bao->id = $cache_id;
    ...
    //
    // finally do it 3rd time, run reconciliation then test expected outcomes.
    //
    $audience->checkForDataUpdates(NULL);
    $bao = new CRM_Mailchimpsync_BAO_MailchimpsyncCache();
    $bao->id = $cache_id;
   ...
   *
   * The first time we're called this will add two tags, tag1 and tag2
   * The second time we're called we remove tag1 and leave tag2 alone.
   * The third time we're called we do nothing.
   *
   ...
   *
   */
  public function hookForTagUpdatesCanBeMadeViaHooks($event) {
    $cache_entry = $event->cache_entry;

    // Extract CiviCRM data stored on this entry.
    $data = unserialize($cache_entry->civicrm_data);

    // At first this won't exist.
    if (empty($data['testHook'])) {
      $data['testHook'] = ['stage' => 1];
    }

    $our_data = &$data['testHook'];
    $changes = FALSE;
    unset($our_data['tag_updates']);
    unset($our_data['mailchimp_updates']);

    if ($our_data['stage'] == 1) {
      // First update: add 2 tags
      $our_data['tag_updates'] = [
        ['name' => 'tag1', 'status' => 'active'],
        ['name' => 'tag2', 'status' => 'active'],
      ];
      $our_data['stage'] = 2;
    ...
    elseif ($our_data['stage'] == 2) {
      // Next test: remove one, leave the other.
      $our_data['tag_updates'] = [
        ['name' => 'tag1', 'status' => 'inactive'],
      ];
      $our_data['stage'] = 3;
    ...
    if ($changes) {
      $cache_entry->status = 'todo';
    }

    // We always need to save.
